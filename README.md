...
|  # | Theme                    | Tasks                                                                          | Prep |
|---:|:-------------------------|:-------------------------------------------------------------------------------|:----:|
|  1 | Hash Map                 | 706, 1+++, 136, 347+, 451, 49++, 128++, 535, 138, 166, 454                     |  94  |
|  2 | Binary Search            | 704++, 35++, 74+, 153+++, 33+++, 34++, 162+, 875+++                            |  19  |
|  3 | Two Pointers             | 344, 125, 26, 167++, 15++, 11++, 392, 844, 88, 283, 18, 16                     |  89  |
...
|  6 | Intervals                | 56+++, 57++, 253+, 435++, 452+, 763+, 1288, 1094, 436                          |  38  |
|  7 | Stack, Queue             | 20+++, 1047, 933, 2390, 71, 155++, 225, 232, 150+, 227+, 341                   |  67  |
|  8 | Linked List              | 206+++, 21++, 141++, 707, 876, 19+, 234, 24, 2++, 138+, 142, 23+++, 143+, 25+  |  42  |
...
Отлично, переходим к Binary Search Tree (BST). Это структура, где левое поддерево меньше корня, правое — больше. Здесь
важно помнить, что in-order обход даёт отсортированный массив.
...
96. Unique BST Динамическое программирование (количество деревьев). Редко спрашивают напрямую.
95. Unique BST II Генерировать ВСЕ деревья. Сложно и редко.
    Оптимальный порядок решения задач
    Блок 1 — База: поиск и построение (Easy)
700. Search in a BST

Если корень null → null. Если val == root.val → root. Если меньше → идём налево, иначе направо.

108. Convert Sorted Array to BST

build(left, right): если left > right → null. mid = (left+right)/2. Корень = nums[mid]. Левое = build(left, mid-1),
правое = build(mid+1, right).
...
Если null → новый узел. Если меньше → идём налево, иначе направо.
...
Связка "Binary Tree (DFS) + BST" (ваша пара №5)
Эти темы естественно дополняют друг друга:

DFS на обычном дереве — рекурсия и обходы.

BST — добавляет условия упорядоченности (left < root < right).

In-order на BST — ключ ко многим задачам (k-й элемент, проверка валидности).

Совет по подготовке
Сначала 700 и 108 — поиск и построение.

Затем 98 — научитесь проверять валидность (передача min/max).

После 701 и 450 — вставка и удаление (ключевые операции).

Потом 230 и 173 — работа с порядком.

На закуску 538 — reverse in-order.
